...
I dette repoet brukes **`object` med `by lazy`** for delt testoppsett, ikke `@BeforeAll`/`@JvmStatic`
(se `TestApplication.kt`). Foretrekk dette mønsteret i nye tester:
...
> Merk for dp-mellomlagring: repoet har verken TestRapid (Kafka) eller Testcontainers-Postgres —
> bruk `TestApplication.withMockAuthServerAndTestApplication` og `docker-compose up` (fake-gcs-server)
> i stedet, se `.github/copilot-instructions.md`.
